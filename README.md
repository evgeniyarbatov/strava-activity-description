...
2. Add API keys (files are read from `~/gitRepo/[private]`): `openweather.env` with `OPENWEATHER_API_KEY=...`; `tomtom.env` with `TOMTOM_API_KEY=...`; `gemini.env` with `GOOGLE_API_KEY=...`
